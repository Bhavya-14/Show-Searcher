        ...
        for(let result of details){
            if(result.show.image){
                ...
                if (result.show.rating.average) {
                    ...
                    showRating.innerHTML = `<span>Rating</span><br>${result.show.rating.average}/10`
                    detailsDiv.append(showRating);
                };
                //appending genres
                if (result.show.genres.length > 0) {
                    const showGenre = document.createElement('p');
                    showGenre.classList.add('description');    
                    showGenre.innerHTML = `<span>Genres</span><br>${result.show.genres}`
                    detailsDiv.append(showGenre);
                } 
